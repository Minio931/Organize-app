...
import { IconHome, IconCheckbox, IconListDetails, IconCoins, IconLogout } from '@tabler/icons-react';

import useMenu from '../hooks/useMenu';

const AsideNavigation = ({ onClick }) => {
   const { state: activeState, activeHandler } = useMenu(['isHome', 'isHabit', 'isTask', 'isBudget']);
            ...
            <ul className={classes['nav-list']}>
               <div className={classes['nav-main']}>
                  <li onClick={homeHandler} className={classes['nav-item']}>
                     <Link className={classes['nav-link']} to="/organize-app/dashboard">
                        <IconHome className={homeClass} />
                     </Link>
                  </li>
                  <li onClick={habitHandler} className={classes['nav-item']}>
                     <Link className={classes['nav-link']} to="/organize-app/habit-tracker">
