...
const createTodo = async (todo) => {
   const { userId, name, description, creationDate, executionDate, completion } = todo;
...
const getTodos = async (userId) => {
   const result = await db.query('SELECT * FROM todos WHERE user_id = $1 ORDER BY id ASC', [userId]);
...
const getTodayTodos = async (userId) => {
   ...
   const result = await db.query('SELECT * FROM todos WHERE user_id = $1 AND execution_date = $2 ORDER BY id ASC', [
      userId,
      todayDate,
   ]);
...
const editTodo = async (todo) => {
   const { id, name, description, executionDate, completion } = todo;

   const result = await db.query(
      'UPDATE todos SET name = $1, description = $2, execution_date = $3, completion = $4 WHERE id = $5 RETURNING *',
      [name, description, executionDate, completion, id],
   );
   if (result.rows.length === 0) {
      throw new TodoNotFoundError('Todo not found');
   }
...
const updateTodo = async (todo) => {
   ...
   const result = await db.query('UPDATE todos SET completion = $1 WHERE id = $2 RETURNING *', [completion, id]);
