...
const TodoView = ({ tasks }) => {
   ...
   const resize = (event) => {
      notDoneContainer.current.style.height = `${parseInt(initialSize) + parseInt(event.clientY - initialPos)}px`;

      doneContainer.current.style.height = `${
         parseInt(initialSize2) - parseInt(notDoneContainer.current.offsetHeight - initialPos + 148)
      }px`;
   };
   return (
      <div className={classes['todos-wrapper']}>
         ...
         <div className={classes['todo-container']}>
            <div ref={notDoneContainer} className={classes['not-done-container']} onDragStart={initial}>
               <ul className={classes['todos-list-not-done']}>
                  {todos.length === 0 && <li className={classes['empty-todo']}>No todos for today</li>}
                  {undoneTodos.map((todo) => {
                     return (
                        <li className={classes['todo-item']} key={todo.id}>
                           <input type="checkbox" id={todo.id} onChange={onClickHandler} checked={todo.completion} />
            ...
            <div className={classes['todo-divider']}>
               <div onDragStart={initial} draggable="true" onDrag={resize} className={classes['todo-divider-line']}>
            ...
            <div ref={doneContainer} className={classes['done-container']}>
               <ul className={classes['todos-list-done']}>
                  {doneTodos.map((todo) => {
                     return (
                        <li className={classes['todo-item']} key={todo.id}>
                           <input type="checkbox" id={todo.id} checked={todo.completion} onChange={onClickHandler} />
                           <label htmlFor={todo.id}></label>
                           <p>{todo.name}</p>
                        </li>
                     );
            ...
            <div className={classes.proggress}>
               <div className={classes['proggress-bar-wrapper']}>
                  <ProgressBar className={classes.completion} fillPercent={tasksProgress} />
